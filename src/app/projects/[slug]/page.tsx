  ...
  return (
    <>
      ...
      <div className="text-center flex flex-col items-center gap-4">
        <p className="text-sm md:text-lg tracking-tight text-muted-foreground">{frontmatter.category}</p>
        <h1 className="text-5xl md:text-7xl font-semibold tracking-tighter uppercase">{frontmatter.title}</h1>
        <p className="max-w-5xl hidden md:block text-2xl tracking-tighter">{frontmatter.description}</p>

        <div className="tracking-tight py-2 px-4 bg-neutral-900 text-foreground rounded-lg">
          <ProjectInfoModal project={frontmatter} />
        </div>
      </div>

      {/* Hero / Main Thumbnail Image */}
      <div className="container mx-auto px-4 flex flex-col gap-4 pt-12">
